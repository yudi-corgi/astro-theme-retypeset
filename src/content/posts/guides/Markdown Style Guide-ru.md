...
Чтобы создать цитату, добавьте символ `>` и пробел перед текстом. Цитаты могут содержать несколько абзацев. Для указания источников используйте теги `<cite>` или `<footer>`, а сноски можно вставить с помощью синтаксиса `[^1]` или `[^note]`.
...
Чтобы создать блок кода, оберните ваш код тремя обратными апострофами ```` ``` ````. Вы можете указать язык программирования после открывающих обратных апострофов, чтобы указать, как раскрашивать и стилизовать ваш код, например: html, javascript, css, markdown и т.д.
